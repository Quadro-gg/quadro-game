...
(function() {
    ...
    function checkForWords() {
        ...
        for (let r = NUM_ROWS - 1; r >= 0; r--) {
            ...
            if (isFullRow) {
                ...
                if (matchedWord) {
                    console.log(`Word found: ${matchedWord} in row ${r}`);
                    rowsToClear.push(r);
        ...
        if (rowsToClear.length > 0) {
            updateGridUI();
            setTimeout(() => removeClearedRowsAndShift(rowsToClear), WORD_CLEAR_DELAY);
    ...
    function removeClearedRowsAndShift(rowsToRemove) {
        ...
        const clearedRowCount = rowsToRemove.length;
        let scoreMultiplier = clearedRowCount;
        score += 10 * scoreMultiplier * scoreMultiplier;
        updateScoreUI();
        if (currentGameMode === GAME_MODE.DAILY_CHALLENGE) {
            rowsClearedSinceSpeedIncrease += clearedRowCount;
        ...
        let newGridRow = NUM_ROWS - 1;
        for (let r = NUM_ROWS - 1; r >= 0; r--) {
            if (!rowsToRemove.includes(r)) {
                if (newGridRow >= 0) {
                    for (let c = 0; c < NUM_COLS; c++) {
                        if (grid[r]?.[c]) {
                            newGrid[newGridRow][c] = grid[r][c];
                            if(newGrid[newGridRow][c]) newGrid[newGridRow][c].clearing = false;
                        }
                    }
                    newGridRow--;
                }
